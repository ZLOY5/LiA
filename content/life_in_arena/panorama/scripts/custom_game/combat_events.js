...
function CreateToast(data) {
	...
	if (data.eventType == 1) {
		...
		if (data.killerPlayer == -1)
			var text = $.Localize("#Monsters") + colorEnd + killIcon + heroKilledIcon + killedColor + playerKilledName;
		else
			var text =
				heroKillerIcon +
				killerColor +
	...
	if (data.eventType == 2) {
		if (data.killerPlayer == localPlayerID) newToast.AddClass("LocalPlayerInvolved");

		if (data.killerPlayer == localPlayerID) newToast.AddClass("AllyEvent");
		else newToast.AddClass("NeutralEvent");
		...
		var text = heroKillerIcon + killerColor + playerKillerName + colorEnd + killIcon + $.Localize("#Boss");
	...
	if (data.eventType == 4) {
		...
		var text = fromIcon + firstColor + fromPlayerName + colorEnd + Icon + toPlayerIcon + secondColor + toPlayerName;
	...
	$.Schedule(10, function () {
		newToast.RemoveClass("ToastVisible");
		newToast.AddClass("Collapse");
	});
}
