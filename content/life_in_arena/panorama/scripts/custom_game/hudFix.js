...
(function () {
	...
	var randomHeroButton = dotaHud
		.FindChildTraverse("PreGame")
		.FindChildTraverse("HeroPickScreen")
		.FindChildTraverse("RandomButton");
	randomHeroButton.style.saturation = 1;
	randomHeroButton.style.brightness = 1;
	randomHeroButton.SetPanelEvent("onactivate", function () {
		GameEvents.SendCustomGameEventToServer("lia_random_hero", {});
	});
	randomHeroButton.SetPanelEvent("onmouseover", function () {
		randomHeroButton.ClearPropertyFromCode("brightness");
		randomHeroButton.style.brightness = 1.5;
	});
	randomHeroButton.SetPanelEvent("onmouseout", function () {
		randomHeroButton.ClearPropertyFromCode("brightness");
		randomHeroButton.style.brightness = 1;
	});

	var reRandomHeroButton = dotaHud
		.FindChildTraverse("PreGame")
		.FindChildTraverse("HeroPickScreen")
		.FindChildTraverse("ReRandomButton");
	reRandomHeroButton.SetPanelEvent("onactivate", function () {
		GameEvents.SendCustomGameEventToServer("lia_random_hero", {});
	});

	reRandomHeroButton = dotaHud
		.FindChildTraverse("PreGame")
		.FindChildTraverse("StrategyScreen")
		.FindChildTraverse("EnterGameReRandomButton");
	reRandomHeroButton.SetPanelEvent("onactivate", function () {
		GameEvents.SendCustomGameEventToServer("lia_random_hero", {});
	});
...
function ValvePlzFix() {
	var iconFixer = function () {
		...
		if (Game.GameStateIsAfter(DOTA_GameState.DOTA_GAMERULES_STATE_PRE_GAME - 1)) {
			...
			if (popularItems != null) popularItems.style.visibility = "collapse";
		}
		...
		if (statsTooltip != null) {
			...
			if (!isNaN(strength)) {
				...
				statsTooltip
					.FindChildTraverse("StrengthDamageLabel")
					.SetDialogVariableInt("primary_attribute_damage", strength * 1.5);
			}
			...
			if (!isNaN(agility)) {
				...
				statsTooltip
					.FindChildTraverse("AgilityDamageLabel")
					.SetDialogVariableInt("primary_attribute_damage", agility * 1.5);
			}
			...
			if (!isNaN(intellect)) {
				...
				statsTooltip
					.FindChildTraverse("IntelligenceDamageLabel")
					.SetDialogVariableInt("primary_attribute_damage", intellect * 1.5);
			}
		}
	};
	iconFixer();
