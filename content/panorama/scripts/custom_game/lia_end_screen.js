...
function OnUpdActionEnd(data) {
	...
	if (playersContainer) {
		for (var playerID of teamPlayers) {
			...
			_ScoreboardUpdater_UpdatePlayerPanel(
				_endScoreboardHandle.scoreboardConfig,
				playersContainer,
				playerID,
				localPlayerTeamId,
			);
	...
	if (winningTeamId === 3) {
		...
		if (3000 - sec > 0) extraForTime = (3000 - sec) * 1;
		if (extraForTime > 1500) extraForTime = 1500;
	}
	...
	var strokaMin = "";
	var strokaHour = "";
	if (hour < 10) strokaHour = "0" + hour;
	else strokaHour = hour;
	if (min < 10) strokaMin = "0" + min;
	else strokaMin = min;
	if (sec < 10) strokaSec = "0" + sec;
	else strokaSec = sec;
	//
	if (hour !== 0) endScreenTime.text = $.Localize("#LiaTime") + strokaHour + ":" + strokaMin + ":" + strokaSec;
	else endScreenTime.text = $.Localize("#LiaTime") + strokaMin + ":" + strokaSec;
}
