...
function GlyphClick() {
	if (!Players.IsReadyToRound(Game.GetLocalPlayerID())) GameEvents.SendCustomGameEventToServer("glyph_clicked", {});
}
...
function OnPlayerStateChanged(table, key, data) {
	...
	if (key == "Player" + Game.GetLocalPlayerID()) {
		if (data.readyToRound) glyphButton.AddClass("ReadyToRound");
		else glyphButton.RemoveClass("ReadyToRound");
	}
}
