...
function OnUpdateHeroSelection() {
	for (var playerId of Game.GetAllPlayerIDs()) {
		if (!Players.IsSpectator(playerId)) UpdatePlayer(playerId);
	}
}
...
function UpdatePlayer(playerId) {
	...
	if (playerInfo.player_selected_hero !== "") {
		...
		playerPortrait.SetPanelEvent("onmouseover", function () {
			playerPanel.ToggleClass("ShowHeroName");
		});
		playerPortrait.SetPanelEvent("onmouseout", function () {
			playerPanel.ToggleClass("ShowHeroName");
		});
	...
	if (playerInfo.player_steamid != 90071996842377222) {
		...
		playerName.SetPanelEvent("onmouseout", function () {
