...
function CheckHeroName() {
	...
	} else withoutHero = true;

	if (withoutHero) $.Schedule(0.5, CheckHeroName);
...
function ChangeGold(bIncrement) {
	var addGold = 10;
	if (GameUI.IsShiftDown()) addGold = 100;

	if (bIncrement) context.gold += addGold;
	else context.gold -= addGold;
	...
	if (context.gold < 0) context.gold = 0;
...
function ChangeLumber(bIncrement) {
	var addLumber = 1;
	if (GameUI.IsShiftDown()) addLumber = 10;

	if (bIncrement) context.lumber += addLumber;
	else context.lumber -= addLumber;
	...
	if (context.lumber < 0) context.lumber = 0;
...
function SharedHeroToggle() {
	GameEvents.SendCustomGameEventToServer("shared_hero_toggle", {
		togglePlayerID: context.playerID,
		state: Boolean($.FindChildInContext("#Hero").checked),
	});
}

function SharedUnitsToggle() {
	GameEvents.SendCustomGameEventToServer("shared_units_toggle", {
		togglePlayerID: context.playerID,
		state: Boolean($.FindChildInContext("#Units").checked),
	});
}

function DisableHelpToggle() {
	GameEvents.SendCustomGameEventToServer("disable_help_toggle", {
		togglePlayerID: context.playerID,
		state: Boolean($.FindChildInContext("#DisableHelp").checked),
	});
}
...
(function () {
	...
	if (heroName != "") playerNameLabel.text = playerNameLabel.text + " | " + heroName;
	else {
		needHeroName = true;
	}

	$.FindChildInContext("#PlayerRowContainer").style.backgroundColor =
		"gradient( linear, 0% 0%, 110% 0%, from( #00000000 ), color-stop( 0.2, " +
		playerColor +
		"44 ), to( #00000000 ) );";
	...
	if (needHeroName) $.Schedule(0.5, CheckHeroName);
})();
