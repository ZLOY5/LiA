...
function StartTimer(data) {
	...
	if (schedule == null) Tick();
}

function StopTimer() {
	//$.Msg("StopTimer")
	if (schedule != null) $.CancelScheduled(schedule);
	schedule = null;
...
function OnTimerChanged(table, key, data) {
	if (key == "lia_timer") {
		if (data.startTime != startTime) StartTimer(data);
