...
function modifier_damage_return_lua:IsHidden()
	return true
end

function modifier_damage_return_lua:DeclareFunctions()
	local funcs = {
...
function modifier_damage_return_lua:OnAttackLanded(params)
	if IsServer() then
		if params.target == self:GetParent() and ( not self:GetParent():IsIllusion() ) then
			...
			ApplyDamage({victim = params.attacker, 
						 attacker = target, 
